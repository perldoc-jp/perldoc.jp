...
- **data/years.pl の自動更新 (年次作業は不要)**: databuild は `create_data.pl`
  ...
  過去年を再導出すると、その年の `in` (翻訳者ページに出る dist 名) が現在の
  命名規則に書き換わる。`_file2name` がハイフンを 1 個だけ `::` にしていた頃の
  値が凍結されているため (例: `IO::Socket-SSL` → `IO::Socket::SSL`)。
  表示だけの差で、件数や翻訳者ごとの集計は変わらない (同一性の判定は
  `PJP::M::YearData::_dedup_in` が両方の表記を同じものとして扱う)。
  2011 年より前の分は再導出できないので、この 2 つの表記はファイル内に
  混在したままになる
