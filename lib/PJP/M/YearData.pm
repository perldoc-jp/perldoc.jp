...
# git 履歴から再導出できる最古の年。これより前の統計は、CVS と複数の旧リポジトリを
# 当時のシステムで観測した結果の凍結で、現在の git 履歴からは再現できない。
# 現行のパイプラインが実際に git だけで導出した最初の年がここ (それより前は
# 訳者名も件数も別系統の記録で、再導出は「復元」ではなく別の指標への置換になる)
use constant GIT_DERIVABLE_SINCE => 2023;
...
sub build {
    ...
    # 書き戻され、以後のビルドでも残り続ける)。GIT_DERIVABLE_SINCE より前の統計は、
    # CVS と複数の旧リポジトリを当時のシステムで観測した結果を凍結したもので、
    # 現在の git 履歴からは再現できない。
